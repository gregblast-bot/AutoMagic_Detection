# Magic: The Gathering (MTG) card detection example. This is adapted slightly from the person detection example.
...
network to recognize MTG cards in images.
...
camera, you'll need to implement your own `arduino_image_provider.cpp` code. It also has a set of LEDs, which are used to indicate whether a MTG card has been recognized.

Note: This code supports OV7675, still testing implementation.
...
it and click `mtg_detection` to load the example.

Use the Arduino IDE to build and upload the example. Once it is running, you
should see the built-in LED on your device flashing. The built-in LED will flash on/off for each inference cycle.  The green LED will be lit if a MTG card is predicted, 
The blue LED will be lit if the prediction is not-a-mtgc.
...
MTG score: 39.6% No MTG score: 60.93%
```
...
[training_a_model.md](https://github.com/tensorflow/tflite-micro/blob/main/tensorflow/lite/micro/examples/mtg_detection/training_a_model.md) for instructions.
